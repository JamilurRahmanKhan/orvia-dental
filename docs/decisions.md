...
## 2026-09-17 — Wide-viewport layouts for narrow-column interior pages, sourced from real reference patterns

The round-3 critique flagged five interior pages running a single narrow text column against a mostly-empty right half at 1280/1920: `EmergencyDentist.vue`, `TreatmentDetail.vue`, `MembershipPlan.vue`, `TeamMember.vue`, `Gallery.vue`. Per the client's ask to pull from real component libraries rather than guess, checked [untitledui.com/components/blog-post-sections](https://www.untitledui.com/components/blog-post-sections) directly — its own description: "blog post sections are the article page itself... ten content section layouts, and the content items that build the body in a 720px column" — confirming the pattern to translate: a fixed-width article column running alongside a **persistent sidebar** (not a wider column), which is what all five pages below now do. Everything is scoped CSS grid, no new dependency.

- **`EmergencyDentist.vue`**: wrapped the six content blocks in `.page__main` (unchanged, ~70ch) and added `.page__aside` — a sticky "Right now" card (open/closed status, emergency-exam price, same-day availability, book/call buttons) plus a related-links list. Was previously a flat list of sections against a bare background at wide viewports.
- **`TreatmentDetail.vue`**: converted seven stacked full-bleed alternating-background sections (What it is / Who it's for / What it's like / Cost / Insurance / Other options / Aftercare / FAQ) into one `.td-article__main` column with hairline dividers between subsections, alongside a sticky `.td-article__aside`: an "At a glance" facts card (visits/recovery/insurance, moved out of its own separate full-width band), a book/call CTA, and a pull-quote card (`content.whoItsFor[0]`, e.g. "You have one or more missing teeth") on the accent-tint background — a stat/pull-quote callout, the second pattern named in the brief. Removed the standalone `.td-facts` band since its content now lives in the sidebar.
- **`MembershipPlan.vue`**: split "How it works" into a main column (the three numbered steps) plus a sticky sidebar with the membership price card and a new stat callout ("15% off every other treatment..."). The price card and stat previously sat stacked full-width below the steps.
- **`TeamMember.vue`**: the bio paragraph now runs the full width of its content column instead of being capped at `--measure-narrow` inside an already-narrow half, with the languages/treatments/book-CTA card sitting beside it (`minmax(0,1fr) 280px`) instead of stacked below in the same constrained width.
- **`Gallery.vue`**: the portrait-oriented before/after photos (698×894) were forcing each case card to an awkward height when stretched across a wide grid track — capped the viewer at 420px (matching the fix already applied to the homepage's `BeforeAfterGallery.vue` in an earlier round, now applied here too) and gave the notes column real content to fill the resulting width: a cost figure and a "What [treatment] involves" link into the matching treatment page, instead of empty space below the chart.

All five verified at 375/768/1280/1920 with no horizontal overflow, `npm run build` clean, no console errors.
